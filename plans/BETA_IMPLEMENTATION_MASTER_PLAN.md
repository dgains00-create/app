...
| **B3** (P2-T07 Boquilhas contract) | **RESOLVED — PLAN ACCEPT** (`plans/contracts/P2-T07_BOQUILHAS_CONTRACT.md` corrected at `b884dd84…`; authoring response `dev/responses/P2_T07_CONTRACT_AUTHORING_RESPONSE.md`; 83 AC / 86 matrix rows; 6 tables / 1 migration; 18 routes; 0 BLOCKING / 0 REQUIRES OWNER / 12 NON-BLOCKING pinned defaults). Architect plan review `542a08a1bcf1340306f8e337a6c597580a921f3d` = **PLAN REJECT — B1 only**; the B1 correction (two partial unique active-anchor indexes + exact 23505 → `Refused(ActiveAggregateExists)` mapping + concurrent create/reopen coverage) was applied; the focused Architect PLAN re-review `7c2479ebae50f8fe18a770fd373cffaae65a48e1` = **PLAN ACCEPT** (implementation AUTHORIZED against `b884dd84…` only); **B3 CLOSED as a blocker; P2-T07 IMPLEMENTED** (see §7 P2-T07 status block) — awaiting independent verification / Architect implementation review. |
...
- **CONTRACT STATUS: IMPLEMENTED — AWAITING INDEPENDENT VERIFICATION / ARCHITECT IMPLEMENTATION
  REVIEW.** The B3 contract is `plans/contracts/P2-T07_BOQUILHAS_CONTRACT.md` (corrected at
  `b884dd84…`, **PLAN ACCEPT** at the focused Architect B1 re-review `7c2479eba…`, dmo-work —
  implementation AUTHORIZED against `b884dd84…` only). It fixes — over the
  ...
  **0 BLOCKING, 0 REQUIRES OWNER DECISION, 12 NON-BLOCKING with pinned defaults** —
  ...
  green on a fresh disposable PostgreSQL. **P2-T07 is NOT self-verified and NOT closed**: it
  awaits independent verification / the Architect implementation review. P2-T08 / P2-T10 remain
...
| 9.13 Boquilhas | MISSING | READY FOR IMPLEMENTATION | P2-T07 — **IMPLEMENTED — AWAITING INDEPENDENT VERIFICATION / ARCHITECT IMPLEMENTATION REVIEW** (contract `plans/contracts/P2-T07_BOQUILHAS_CONTRACT.md` corrected at `b884dd84…`; PLAN ACCEPT (focused B1) `7c2479ebe…`; response `dev/responses/P2_T07_IMPLEMENTATION_RESPONSE.md`; 83 AC / 86 matrix rows incl. K6/K7/K8; 6 tables / 1 migration 007; 18 routes; implementation green on a fresh disposable PostgreSQL; `CurrentBuildAvailable` `[]`) |
| 9.14 Boquilhas repairer | MISSING | READY FOR IMPLEMENTATION | P2-T07 — **IMPLEMENTED — AWAITING INDEPENDENT VERIFICATION / ARCHITECT IMPLEMENTATION REVIEW** (consumption contract implemented: machine → current assignment → resolved suggestion; final selected `repairer_id` stored on external Saída with historical retention; register/assignments owned by `Controlo_Create → Definições`; same contract as 9.13) |
