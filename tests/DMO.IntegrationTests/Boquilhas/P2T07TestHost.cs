...
/// <summary>
/// P2-T07 test host composition: the real web application with the access, account and
...
/// P2-T04/P2-T05/P2-T06 compositions: production availability is never edited, and
/// <c>ModuleRegistrations.CurrentBuildAvailable</c> stays empty in the real composition. The
/// Boquilhas repository, the bq-context traversal seam and the consumed P2-T05 reads run against
/// the accepted <see cref="P2T07TestStore"/>; the P2-T04 contracts (Job On, Tool) run against the
/// REAL services over the accepted <see cref="P2T04TestStore"/>, so the composition rows prove the
...
/// </remarks>
internal static class P2T07TestHost
{
    /// <summary>The test-only authentication scheme name.</summary>
    public const string SchemeName = "dmo.p2t07.test.auth";

    /// <summary>The header that marks a request as authenticated.</summary>
    public const string HeaderName = "X-Dmo-P2T07-Test-Auth";

    /// <summary>The fixed backend actor of every granted USER request (assertible, AC-E5).</summary>
    public static readonly Guid ActorUserId = Guid.Parse("bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb");

    /// <summary>The Boquilhas Module made available by the controlled test registry.</summary>
    public static IReadOnlyList<ModuleDefinition> AvailableModules { get; } =
    [
        TestNavigationComposition.Definition(
            ModuleCatalog.Boquilhas, "Boquilhas", "boquilhas", "Boquilhas"),
    ];

    /// <summary>
    /// Creates the test host: the supplied caller, the granted Modules, and fresh in-memory stores
    /// behind the real P2-T07 + P2-T04 services.
    /// </summary>
    public static WebApplicationFactory<Program> Create(
        CurrentAccount current,
        IReadOnlyList<ModuleDefinition> granted,
        P2T07TestStore? store = null,
        Action<IServiceCollection>? additional = null)
    {
        var baseFactory = new DmoWebApplicationFactory();
        var composition = store ?? new P2T07TestStore();
        ...
        return TestNavigationComposition.ConfigureFactory(
            ...
            services =>
            {
                // The Boquilhas + consumed P2-T05 repository contracts run against the store.
                services.RemoveAll<IBoquilhasRepository>();
                services.RemoveAll<IBoquilhasContextRead>();
                services.RemoveAll<IRepairerRepository>();
                services.RemoveAll<IMachineRepairerAssignmentRepository>();

                services.AddSingleton<IBoquilhasRepository>(composition);
                services.AddSingleton<IBoquilhasContextRead>(composition);
                services.AddSingleton<IRepairerRepository>(composition);
                services.AddSingleton<IMachineRepairerAssignmentRepository>(composition);
                ...
                services.AddSingleton<IToolRepository>(composition.JobOnToolStore);
                services.AddSingleton<IJobOnRepository>(composition.JobOnToolStore);
    ...
    /// <summary>The granted Boquilhas Module.</summary>
    public static IReadOnlyList<ModuleDefinition> AllGranted() => AvailableModules;

    /// <summary>An unrelated-grant set (job-on-view), for the sibling-denial rows (AC-A2).</summary>
    public static IReadOnlyList<ModuleDefinition> UnrelatedOnly() =>
    [
        TestNavigationComposition.Definition(
            ModuleCatalog.JobOnView, "Job On View", "job-on", "Job On"),
    ];
